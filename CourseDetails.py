...
coursetitle = form.getvalue('course')

cgitb.enable()
dbconn = pymysql.connect(host="localhost", user="root", password="", database="dkast_py_site")
cur = dbconn.cursor()

query = ("""select * from Emp_Inventory where VidTitle = '%s' """ % coursetitle)
cur.execute(query)
data = cur.fetchall()
        ...
        <div class="container-fluid ">
            ...
            <div class="collapse navbar-collapse d-lg-flex justify-content-end" id="navbar-in">
                <div class="navbar-nav gap30px">
                    ...
                    <div class="nav-item">
                    <!-- <button class="signin btnbg" type="button" data-bs-target="#signinbtn"
...
for i in data:
    ...
    print("""            
            <div class="content-1">
                <div class="chap border border-success border-opacity-25 rounded p-3 shadow-lg">
                    <div class="box1 p-5">
                        <h4>%s</h4>
                    </div>
                    <div class="box2 p-3">
...
print("""            
                <div class="chap border border-danger border-opacity-25 courseactive d-none rounded p-3">
                    <div class="box1 p-5">
                        <h4>Chapter-1</h4>
                    </div>
                    <div class="box2 p-3">
                        <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Quo id rerum cupiditate
                            voluptatibus culpa? Quis necessitatibus at, suscipit dolore provident minima. Vero fugiat
                            tenetur mollitia animi. Eveniet provident corrupti deleniti?
                        </p>
                    </div>
                </div>
    ...
    <section class="course py-5">
        <div class="pt-5">
            <div class="enrollsec  p-5">                
                    <div class="row">
                        <div class="col-6 d-flex justify-content-end">
                            <label for="mailid" class="form-label">
                                Click the Button to Join the Course 
                            </label>
                        </div>
                        <div class="col-6 d-flex gap-3">                            
                            <button class="btn add-btn" type="button" data-bs-target="#signinbtn" data-bs-toggle="modal">Enroll Now</button>
                        </div>
                    </div>                
            </div>
        </div>
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-6">
                                <ul>
                                    ...
                                    <li>Sign Up</li>
                                    <li>Sign In</li>
                                    <li><a id="stlogbt" type="button" data-bs-target="#stafflogin"
                                            data-bs-toggle="modal">Staff login</a></li>
                                    <li>Pricings</li>
                                    <li>Topics</li>
                                </ul>
                            </div>
...
if usrsigninbtn != None:
    try:
        ...
        if usrname != None and usrpass != None:
            fquery = ("""select id,userid from user_details where useremail = '%s' and password = '%s'""" % (usrname, usrpass))
            cur.execute(fquery)
            ...
            for item in getdata:
                ...
                getuid = item[1]
            if getid != None:
                cquery = ("""INSERT INTO course_enroll (CourseName, CourseStartDate, CourseEndDate, Status, Empid) VALUES('%s', '%s', '%s', '%s', '%s')""" % (coursetitle, today, enddate, status[0], getuid))
                cur.execute(cquery)
                if cquery:
                    print("""
                    <script>
                       alert("login Successfully");
                       location.href="./UserCourseList.py?id=%s";
                    </script>
                    """ % getid)
                dbconn.commit()
                dbconn.close()
        ...
        print("""
                <script>
                    alert("EmailId and Password is incorrect.... and \n/n %s ");
                    $(document).ready(function(){
                        // Open the modal
                        $('#signinbtn').modal('show');
                    });
